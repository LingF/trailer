...
Promise.resolve().then(() => {
  ...
  setImmediate(() => console.log('[阶段3.immediate] promise 回调1 新增的 immediate 回调4'))
})

readFile('../package.json', 'utf-8', data => {
  console.log('[阶段2...IO 回调] 读文件的回调1')

  readFile('../video.mp4', 'utf-8', data => {
    console.log('[阶段2...IO 回调] 读文件的回调2')

    setImmediate(() => console.log('[阶段3.immediate] 读文件回调2 新增的 immediate 回调4'))
  })
  ...
  setImmediate(() => {
    ...
    console.log('[...待切入下一阶段] 正在同步堵塞地读一个大文件')
    ...
    readFile('../video.mp4', 'utf-8', data => {
      console.log('[阶段2...IO 回调] 读文件的回调3')

      setImmediate(() => console.log('[阶段3.immediate] 读文件回调3 新增的 immediate 回调6'))
      setTimeout(() => console.log('[阶段1....定时器] 读文件回调3 新增的 定时器 回调8'), 0)
    })
  })
